...
if [ "$first" != 1 ];then
        if [ ! -f $tarball ]; then
                ...
                *)
                ...
                wget "https://archives.fedoraproject.org/pub/archive/fedora/linux/releases/${fedora}/Container/${archurl}/images/Fedora-Container-Base-${fedora}-1.6.${archurl}.tar.xz" -O $tarball
        fi
        cur=`pwd`
        mkdir -p "$folder"
